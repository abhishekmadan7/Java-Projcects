...
public class BinarySearchTree
{
   ...
   {  	
     if(t==null)
      ...
      TreeNode j = t;
      while(true){
         if(s.compareTo((String)j.getValue()) <= 0)
         {
            if(j.getLeft() == null)
            {
               j.setLeft(new TreeNode(s));
               break;
            }
            j = j.getLeft();
         }
   ...
   public static String display(TreeNode t, int level)
   {
      ...
      if(t == null)
         return "";
      s += display(t.getRight(), level + 1);
      for(int k = 0; k < level; k++)
         s += "\t";
      s += t.getValue() + "\n";
      s += display(t.getLeft(), level + 1);
      return s;
   }
   	/********************
